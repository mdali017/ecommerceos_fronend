...
export function AdminPosProductCard({
  ...
  return (
    <article className="flex flex-col overflow-hidden rounded-xl border border-brand-border bg-card shadow-sm transition-shadow hover:shadow-md sm:rounded-2xl">
      <div className="relative aspect-square overflow-hidden bg-brand-gray">
        {image ? (
          ...
          <div className="flex h-full items-center justify-center text-3xl text-muted/40">
            📦
          </div>
        )}
      ...
      <div className="flex flex-1 flex-col p-2.5 sm:p-4">
        ...
        <p className="hidden text-xs text-muted sm:block">{product.category}</p>
        ...
        <p className="mt-0.5 text-[10px] text-muted sm:mt-1 sm:text-xs">Stock: {product.stockQty}</p>
